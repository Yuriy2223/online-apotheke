...
export function Statistics() {
  const stats = [
    { icon: Package, label: "All products", value: "8,430", active: true },
  ...
  return (
    <ul className="grid grid-cols-1 tablet:grid-cols-3 gap-4 mb-6" role="list">
      {stats.map(({ icon: Icon, label, value, active }, index) => (
        <li
          key={index}
          className={`bg-white-true rounded-lg p-6 border transition-all duration-200 cursor-pointer 
            ${active ? "border-green-light" : "border-gray-light"} 
            hover:border-green-light hover:shadow-sm`}
          role="group"
          aria-label={label}
        >
